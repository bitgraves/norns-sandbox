...
-- tune to, for example, A
  local tuneFreq = 55 * math.pow(2.0, 3.0 / 12.0)
...
function init()
  ...
  -- BGUtil.addEngineControlParam(params, { id = "release", min = 0.5, max = 30, warp = 'exp' })
  BGUtil.addEngineControlParam(params, { id = "envAmount" })
  BGUtil.addEngineControlParam(params, { id = "envFreq", min = 2.0, max = 12.0, warp = 'exp' })
  BGUtil.addEngineControlParam(params, { id = "noiseGain", warp = 'exp' })
...
function key(...)
  BGUtil.setlist_key('sideb/sideb', ...)
end
